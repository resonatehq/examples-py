...
def test_deterministic_prompt_generation(scheduler: DSTScheduler) -> None:
    ...
    query = "home depot news"
    scheduler.add(f"query-{query}", prompt_checking.use_case, query=query)
    assert (
...
def test_not_reasonable_responses(scheduler: DSTScheduler) -> None:
    ...
    scheduler.add(f"query-{input_prompt}", prompt_checking.use_case, query=input_prompt)
